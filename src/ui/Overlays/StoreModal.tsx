...
export const StoreModal: React.FC<StoreModalProps> = ({ isOpen, onClose }) => {
  ...
  useEffect(() => {
    ...
    return () => { unsubInv(); unsubStars(); unsubScales(); };
  }, []);
  ...
  const handleBuyBoost = (type: string) => {
    ...
    setConfirmModal({
      ...
      cost: `${cost} 💎`, // Cost in Diamond Eggs
      onConfirm: () => {
  ...
  const renderSkinChain = (name: string, tier: number, max: number, category: any) => {
    ...
    return (
      <div style={{ marginBottom: isMobileLandscape ? 8 : 20, background: '#334155', padding: isMobileLandscape ? '8px' : '15px', borderRadius: 12, border: '1px solid #475569' }}>
        <h4 style={{ margin: '0 0 2px 0', fontSize: isMobileLandscape ? '12px' : '16px', color: '#facc15', fontWeight: 900 }}>{chainTitle}</h4>
        <p style={{ margin: '0 0 6px 0', fontSize: '10px', color: '#94a3b8', lineHeight: '1.2' }}>{chainDesc}</p>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: itemGap }}>
          {[...Array(max)].map((_, i) => {
            ...
            return (
              <div key={i} style={{ display: 'flex', flexDirection: 'column', gap: 2, background: '#1e293b', padding: isMobileLandscape ? '4px' : '10px', borderRadius: 8, border: isOwned ? '1px solid #10b981' : '1px solid transparent' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <span style={{ fontSize: textSize, fontWeight: 'bold', color: isOwned ? '#10b981' : isLocked ? '#64748b' : '#fff' }}>
                ...
                <p style={{ margin: 0, fontSize: '10px', color: isOwned ? '#cbd5e1' : '#94a3b8', opacity: isLocked ? 0.6 : 1 }}>{tierDesc}</p>
              </div>
            );
          })}
        </div>
  ...
  return createPortal(
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.85)', zIndex: 30000, display: 'flex', alignItems: 'center', justifyContent: 'center', backdropFilter: 'blur(8px)', direction: profile.language === 'ar' ? 'rtl' : 'ltr' }}>
      <div style={{ background: '#1e293b', color: '#fff', width: modalWidth, maxWidth: modalMaxWidth, maxHeight: modalMaxHeight, padding: pSize, borderRadius: 24, border: '4px solid #fbc02d', boxShadow: '0 20px 50px rgba(0,0,0,0.5)', display: 'flex', flexDirection: 'column', position: 'relative' }}>
        <h2 style={{ textAlign: 'center', color: '#fbc02d', marginBottom: isMobileLandscape ? 4 : 15, fontSize: hSize, fontWeight: 900, letterSpacing: '1px' }}>{t.store_title}</h2>
        ...
        <div style={{ display: 'flex', gap: 12, justifyContent: 'center', marginBottom: isMobileLandscape ? 8 : 20 }}>
          <div style={{ background: '#0f172a', padding: '4px 10px', borderRadius: 10, display: 'flex', alignItems: 'center', gap: 6, border: '1px solid #334155' }}>
              <span style={{ fontSize: isMobileLandscape ? '14px' : '16px' }}>💎</span><span style={{ fontWeight: 900, color: '#facc15', fontSize: textSize }}>{profile.stars}</span>
          </div>
          <div style={{ background: '#0f172a', padding: '4px 10px', borderRadius: 10, display: 'flex', alignItems: 'center', gap: 6, border: '1px solid #334155' }}>
              <span style={{ fontSize: isMobileLandscape ? '14px' : '16px' }}>🐉</span><span style={{ fontWeight: 900, color: '#10b981', fontSize: textSize }}>{profile.scales}</span>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', borderBottom: '2px solid #334155', marginBottom: isMobileLandscape ? 8 : 20 }}>
          {['stars', 'boosts', 'skins'].map(tab => (
            <button 
              key={tab}
              onClick={() => { soundService.playButtonClick(); setActiveTab(tab as any); }}
              style={{ 
                flex: 1, padding: isMobileLandscape ? '6px' : '12px', border: 'none', background: 'transparent', 
                color: activeTab === tab ? '#fbc02d' : '#64748b', 
                fontWeight: 900, fontSize: textSize, borderBottom: activeTab === tab ? '4px solid #fbc02d' : 'none',
            ...
            <div>
              ...
              {STORE_PRICES.DIAMOND_PACKS.map(p => (
                <div key={p.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: '#334155', padding: itemPad, marginBottom: itemGap, borderRadius: 12, border: '1px solid #475569' }}>
                  ...
                  <button onClick={() => handleBuyPack(p)} style={{ background: '#fbc02d', border: 'none', color: '#000', padding: '4px 12px', borderRadius: 8, fontWeight: 900, cursor: 'pointer', fontSize: textSize, boxShadow: '0 2px 0 #b45309' }}>
                    {p.costXtr} ⭐
                  </button>
                </div>
              ))}
                ...
                <div key={key} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: '#334155', padding: itemPad, marginBottom: itemGap, borderRadius: 12, border: '1px solid #475569' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column' }}>
                        ...
                        <span style={{ fontSize: '10px', color: '#38bdf8' }}>{t.owned_cnt}{profile.inventory[key as keyof typeof profile.inventory] || 0}</span>
                    </div>
                  </div>
                  <button 
                    onClick={() => handleBuyBoost(key)}
                    style={{ background: '#0ea5e9', border: 'none', color: '#fff', padding: '4px 12px', borderRadius: 8, fontWeight: 900, cursor: 'pointer', fontSize: textSize, boxShadow: '0 2px 0 #0369a1' }}>
                    {cost} 💎
                  </button>
                </div>
              ))}
              ...
              <div style={{ background: 'linear-gradient(to right, #1e293b, #0f172a)', padding: '8px', borderRadius: 12, border: '2px dashed #facc15', marginBottom: 15, textAlign: 'center' }}>
                ...
                <button 
                    ...
                    style={{ 
                        ...
                        border: 'none', fontWeight: 900, cursor: profile.stars >= 1000 ? 'pointer' : 'default',
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: '#1e293b', padding: '8px', borderRadius: 8 }}>
                    ...
                    {profile.skins.ironBodyOwned ? (
                      ...
                      <button 
                        ...
                        style={{ padding: '4px 12px', borderRadius: 6, background: profile.scales >= STORE_PRICES.SKINS.IRON_BODY ? '#fbc02d' : '#475569', border: 'none', fontWeight: 900, cursor: 'pointer', fontSize: '11px', color: profile.scales >= STORE_PRICES.SKINS.IRON_BODY ? '#000' : '#94a3b8' }}
                      >
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: '#1e293b', padding: '8px', borderRadius: 8 }}>
                    ...
                    {profile.skins.crystalBodyOwned ? (
                      ...
                      <button 
                        ...
                        style={{ padding: '4px 12px', borderRadius: 6, background: profile.scales >= STORE_PRICES.SKINS.CRYSTAL_BODY ? '#fbc02d' : '#475569', border: 'none', fontWeight: 900, cursor: 'pointer', fontSize: '11px', color: profile.scales >= STORE_PRICES.SKINS.CRYSTAL_BODY ? '#000' : '#94a3b8' }}
                      >
          ...
          <div style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.85)', zIndex: 50, display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: 24, padding: 20 }}>
              <div style={{ background: '#0f172a', border: '2px solid #facc15', borderRadius: 16, padding: '24px 16px', width: '100%', maxWidth: 320, textAlign: 'center', boxShadow: '0 10px 25px rgba(0,0,0,0.5)' }}>
                  ...
                  <div style={{ display: 'flex', gap: 12 }}>
                      <button onClick={() => { soundService.playButtonClick(); confirmModal.onConfirm(); }} style={{ flex: 1, padding: '12px', borderRadius: 8, border: 'none', background: '#10b981', color: '#000', fontWeight: '900', cursor: 'pointer', fontSize: '14px' }}>{t.ok}</button>
                      <button onClick={() => { soundService.playButtonClick(); setConfirmModal(null); }} style={{ flex: 1, padding: '12px', borderRadius: 8, border: 'none', background: '#475569', color: '#fff', fontWeight: '900', cursor: 'pointer', fontSize: '14px' }}>{t.cancel}</button>
                  </div>
              </div>
          </div>
        )}
